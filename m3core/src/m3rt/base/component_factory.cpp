...
namespace m3rt
{
    ...
    bool M3ComponentFactory::ReadConfig(const char *filename)
    {
        ...
        if(!out.size()){M3_ERR("[M3ComponentFactory] Failed to read %s, please make sure it exists!\n",filename);return false;}
    ...
    bool M3ComponentFactory::AddComponentLibrary(string lib)
    {
        if(ContainsString(dl_list_str,lib)){
            M3_WARN("[M3ComponentFactory] Library %s already loaded.\n",lib.c_str());
            return true;
        }
        ...
        if(dlib == NULL) {
            M3_WARN("[M3ComponentFactory] Unable to open M3 Component library %s. \nError: %s\n", lib.c_str(), dlerror());
            return false;
        } else {
    ...
    bool M3ComponentFactory::Startup()
    {
        ...
        return false;
        if(dl_list.size() == 0) {
            M3_ERR("[M3ComponentFactory] No M3 Component libraries available\n", 0);
            return false;
        }
    ...
    bool M3ComponentFactory::ReleaseComponent(M3Component *c)
    {
        ...
        M3_WARN("[M3ComponentFactory] Unable to destroy component %s\n", c->GetName().c_str());
        return false;
    }
    ...
    M3Component *M3ComponentFactory::CreateComponent(string type)
    {
        ...
        for(si = dl_types.begin(); si != dl_types.end(); ++si) {
            if((*si).compare(type) == 0) {
                ...
                {
                    M3_ERR("[M3ComponentFactory] Factory : Could not create component %s\n",type.c_str());
                }
                break;
            }
        }
        if(m == NULL) {
            M3_WARN("[M3ComponentFactory] Unable to create component of type %s \n", type.c_str());
        }
        return m;
    }
